...
class TPN_reupload_thumbnails(TPN_post_problems_bot):
	...
	def get_num_of_pages(self):
		num_of_pages_element = WebDriverWait(self.driver, 10).until(ec.visibility_of_element_located((By.CLASS_NAME, 'total-pages')))
	...
	def get_problem_id(self, post):
		text = post.find_element_class_name('row-title').get_attribute('innerHTML').strip()
		return int(text[1])

	def post_thumbnail(self, post, problem_id):
		get_to_post = WebDriverWait(post, 10).until(ec.visibility_of_element_located((By.TAG_NAME, 'a')))
	...
	def start_reuploading_thumbnails(self):
		...
		for index in range(first_index, last_index + 1):
			...
			list_of_posts = WebDriverWait(self.driver, 10).until(ec.visibility_of_element_located((By.ID, 'the-list')))
